...
import { Model, FunctionFactory, SurveyModel } from 'survey-core';
   ...
   },
   {
    ...
    "elements": [
     ...
     },
     {
      "type": "boolean",
      "name": "question6",
      "title": "Haben Sie Schwierigkeiten mit dem Sehen auch wenn Sie eine Brille oder Kontaktlinsen tragen ?"
     },
     {
      "type": "checkbox",
      "name": "question7",
    ...
    "elements": [
     {
      ...
      "imageLink": "https://api.surveyjs.io/private/Surveys/files?name=c2906c76-47e2-4747-9a8f-780547c794dc",
      "imageFit": "cover",
      "imageHeight": "auto",
      "imageWidth": "100%"
     }
    ...
    "elements": [
     {
      ...
      "name": "question11",
      "title": "Diagramm 1",
      "description": "Ich habe meine Freunde letztens gefragt wie oft Sie eigentlich schon verreist sind und wohin. \nDabei haben wir dieses Diagramm erstellt. \n\nBitte schau es dir eine weile an und beantworte dann Fragen dazu."
     },
     {
      type: "landChar",
    },
    ...
    "elements": [
     {
      "type": "comment",
      "name": "question19",
      "description": "Neben den ganzen reisen ist mir auch aufgefallen dass sie relativ viel lesen. Da habe ich mich auch in ihren \nZimmern umgeschaut und die Bücher und Gernre gezählt. "
     },
      {
      type: "bookChar",
    ...
    "elements": [
     {
      "type": "comment",
      "name": "question25",
      "description": "Neben Büchern und Reisen , hören wir auch Musik. Ich hab mit mal ihr Spotify Wrapped angeschaut und das hier\nerstellt : "
     },
     {
      type: "chart",
    },
...
var storeColor = (params: any[]) => {
  const color = params[0];  //ist ein value
  const storeIn = params[1];

  var survey = (this as any).surveyModel;

  console.log('test', color);
  console.log('test2', storeIn);
  console.log('test3', survey);
  
}

FunctionFactory.Instance.register("setOfficialCountryName", storeColor);
@Component({
  selector: 'app-root',
  standalone: true,
...
export class AppComponent implements OnInit {
   ...
   *
   */
  constructor(public service:LoggingService) {
  ...
  pageChange(pageChange: SurveyModel, shit: Survey.CurrentPageChangedEvent) {
    //console.log('shiiitt', shit.oldCurrentPage.id);
    if(shit.oldCurrentPage.id === 'sp_103')
    {
      console.log(localStorage.getItem('colorpick'))
    }

    if(shit.oldCurrentPage.id === 'sp_100' && pageChange.data.Datenschutzerklaerung === false )
    {
